...
priority_queue *
priority_queue_create(size_t key_size,
                      size_t val_size,
                      int (*cmp)(const void *a, const void *b),
                      priority_queue_flags flags)
{
    ...
    if (queue) {
        if (flags & priority_queue_max) queue->max_queue = 1;
        ...
        queue->queue.len = 1;
        queue->queue.arr =
            calloc((queue->queue.cap = 0x400), sizeof *queue->queue.arr);
        if (queue->queue.arr) return queue;
        free(queue);
    }
    return NULL;
}
...
static void
priority_queue_swap(priority_queue *queue,
                    priority_queue_id a,
                    priority_queue_id b)
{
...
priority_queue_bubble_down(priority_queue *queue, priority_queue_id pos)
{
    ...
    while (pos < queue->queue.len) {
        ...
        if (lchild >= queue->queue.len) return;
        ...
        if (rchild < queue->queue.len) {
            queue_CMP(cmp, queue,
                      queue->elements.keys.arr
                          + key_size * queue->queue.arr[lchild],
                      queue->elements.keys.arr
                          + key_size * queue->queue.arr[rchild]);
            if (cmp >= 0) successor = rchild;
        }

        queue_CMP(cmp, queue,
                  queue->elements.keys.arr + key_size * queue->queue.arr[pos],
                  queue->elements.keys.arr
                      + key_size * queue->queue.arr[successor]);
        if (cmp <= 0) return;
...
priority_queue_push(priority_queue *queue, void *key, void *val)
{
    ...
    if (queue->elements.len == queue->elements.cap) {
        ...
        if (cap > queue->elements.max) cap = queue->elements.max;
        if (cap > queue->elements.max) return 0;
        void *tmp;

        tmp =
            realloc(queue->elements.info, cap * sizeof *queue->elements.info);
        if (!tmp) return 0;
        if (queue->elements.info) {
            memset(tmp + queue->elements.cap * sizeof *queue->elements.info, 0,
                   (cap - queue->elements.cap) * sizeof *queue->elements.info);
        }
        else {
        ...
        tmp =
            realloc(queue->elements.keys.arr, queue->elements.keys.size * cap);
        if (!tmp) return 0;
        queue->elements.keys.arr = tmp;

        if (queue->elements.vals.size) {
            tmp = realloc(queue->elements.vals.arr,
                          queue->elements.vals.size * cap);
            if (!tmp) return 0;
    ...
    memcpy(queue->elements.keys.arr + queue->elements.keys.size * id, key,
...
priority_queue_id
priority_queue_get(priority_queue *queue,
                   ...
                   void *val)
{
    ...
    if (id >= queue->elements.len || !queue->elements.info[id].position)
        return 0;
    priority_queue_id pos =
        queue->queue.arr[queue->elements.info[id].position];
    if (key)
        memcpy(key, queue->elements.keys.arr + queue->elements.keys.size * pos,
               queue->elements.keys.size);
    if (val && queue->elements.vals.size)
...
priority_queue_del(priority_queue *queue, priority_queue_id id)
{
    ...
    if (id >= queue->elements.len || !queue->elements.info[id].position)
        return 0;
    if (queue->scan_start > id) queue->scan_start = id;
...
int
priority_queue_update(priority_queue *queue,
                      priority_queue_id id,
                      void *key,
                      void *val)
{
