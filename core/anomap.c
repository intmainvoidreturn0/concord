...
void
anomap_set_on_item_changed(struct anomap *map,
...
anomap_clear(struct anomap *map)
{
    for (size_t i = 0; i < map->map.len; i++) {
        ...
        map->on_changed.cb(
            map, &(struct anomap_item_changed){
          ...
          void *val)
{
    ...
    if (!anomap_index_of(map, key, &mpos)) {
        ...
        if (!_anomap_ensure_capacity(map, map->map.len + 1)) return 0;
        ...
        if (map->vals.size) {
            if (val)
                memcpy(map->vals.arr + val_size * pos, val, val_size);
            else
                memset(map->vals.arr + val_size * pos, 0, val_size);
        }
        if (mpos < map->map.len)
        ...
        if (map->on_changed.cb)
            map->on_changed.cb(map, &(struct anomap_item_changed){
                                        .data = map->on_changed.data,
                                        .op = anomap_insert,
                                        .key = key,
                                        .val.now = val,
    ...
    if (operation & anomap_update) {
        ...
        if (operation & anomap_getval) {
            ...
            if (val_size) {
                if (val)
                    memcpy(val_to_update, val, val_size);
                else
                    memset(val_to_update, 0, val_size);
            }
        }
    }
...
size_t
anomap_copy_range(struct anomap *map,
                  size_t from_index,
                  size_t to_index,
                  void *keys,
                  void *vals)
{
    ...
    if (keys || vals) {
        ...
        for (size_t i = 0;; i++, going_up ? from_index++ : from_index--) {
            ...
            if (keys)
                memcpy(((char *)keys) + key_size * i,
                       map->keys.arr + key_size * pos, key_size);
            if (vals)
                memcpy(((char *)vals) + val_size * i,
                       map->vals.arr + val_size * pos, val_size);
...
size_t
anomap_delete_range(struct anomap *map,
                    size_t from_index,
                    size_t to_index,
                    void *keys,
                    void *vals)
